...
ENV KIEKER_VERSION 1.12-20150822.004707-138
...
RUN \
  wget "${KIEKER_AGENT_BASE_URL}/${KIEKER_AGENT_JAR_SRC}" -O "${KIEKER_AGENT_FOLDER}/${KIEKER_AGENT_JAR}" && \
  wget "${KIEKER_JMETER_URL}" -O "${KIEKER_JMETER_FOLDER}/${KIEKER_JMETER_ZIP}" && \
  unzip ${KIEKER_JMETER_FOLDER}/${KIEKER_JMETER_ZIP} -d /opt && \
  rm ${KIEKER_JMETER_FOLDER} -r && \
